...
# install packages
sudo xbps-install -u xbps && sudo xbps-install -Syu && sudo xbps-install -S git bash-completion stow river yambar xorg seatd elogind foot rofi polkit mesa-dri dunst swaybg swayimg vlc nerd-fonts pipewire neovim neofetch htop fzf lf socklog-void qemu libvirt virt-manager firefox nextcloud-client wget terminus-font groff groff-doc libgroff zathura zathura-pdf-poppler ghostscript pandoc flatpak wl-clipboard hyprpicker waypipe entr grim slurp swappy curl mesa-vaapi alacritty cargo go gvfs-cdaa abcde flac cdparanoia wtype lswt
